...
Ported into molecule-ci's `scripts/` SSOT from
`molecule-core/.gitea/scripts/lint_bp_context_emit_match.py` (Tier 2f,
internal#350). `scripts/` is the canonical location; the byte-identical copy
under `.molecule-ci/scripts/` is the frozen public surface that external
consumers pin (see check-scripts-in-sync.sh, which fails on any drift between
the two). The canonical-name/ci.yml-template renaming
